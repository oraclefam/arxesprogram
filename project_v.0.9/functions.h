...
void store_struct_data_type(char* identifier_data_type){
int i;
    for(i=0;i<20;i++){
    struct_variables[i].sdata_type = identifier_data_type;
    }
}
...
int isDuplicate_struct(char* identifier){
        if(strcmp(identifier,struct_variables[noOfStructVariables].sdata_type) == 0)
            { return 1; }
        else  {return 0;}
}
...
void struct_assign_char_value(char* identifier , char* string_value){
           ...
           if(strcmp(struct_variables[noOfStructVariables].schar_value,"title")==0)
           {struct_variables[noOfStructVariables].schar_value = string_value;
           printf("%s = %s \n ",identifier,struct_variables[noOfStructVariables].schar_value);} 

	   else{printf("\nERROR ON LINE %d : \nInvalid assignment! Expected char identifier \n ",yylineno); exit(0);}

}

void struct_assign_int_value(char* identifier , int int_value){

           if(struct_variables[noOfStructVariables].sint_value==0)
           {struct_variables[noOfStructVariables].sint_value = int_value;
           printf("%s = %d \n ",identifier,struct_variables[noOfStructVariables].sint_value);} 

	   else{printf("\nERROR ON LINE %d : \nInvalid assignment! Expected int identifier \n ",yylineno); exit(0);}

}


void struct_name(char*identifier){
 struct_variables[noOfStructVariables].sname = identifier;
}
...
void store_struct_value_char(char* identifier_name){
int i;
  for(i=0;i<20;i++){
    struct_variables[i].schar_value = identifier_name;
}
}
...
int i;
   for(i=0;i<20;i++){
    struct_variables[i].sint_value = 0;
   }
}
