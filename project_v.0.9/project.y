...
%nonassoc left_parenthesis right_parenthesis left_sbracket right_sbracket
%nonassoc BIGGER_THAN SMALLER_THAN IS_EQUAL NOT_EQUAL AND OR EQ SC EC 
%nonassoc COMMA SEMICOLON DOT AM
...
structure_declaration : STRUCT identifier body_pre struct_body ENDSTRUCT { 
                                  if(!isDuplicate($2)){ 
                                    store_struct_data_type($2);    	
                                  }else{
 printf("\nERROR ON LINE %d : \nInvalid Struct Identifier! , Identifier '%s' already exists\n ",yylineno,$2);
exit(0);                                    
                                  } }
| TYPEDEF STRUCT identifier body_pre body ENDSTRUCT { 
                                  if(!isDuplicate($3)){ 
                                    store_struct_data_type($3);    	
                                  }else{
 printf("\nERROR ON LINE %d : \nInvalid Struct Identifier! , Identifier '%s' already exists\n ",yylineno,$3);
exit(0);                                    
                                  } }
...
struct_variable_int : identifier { store_struct_value_int(); }	

	       | error '>'                   {;}
	       ;

struct_variable_char : identifier	{ store_struct_value_char($1); }
...
use_struct: body_pre STRUCT identifier identifier SEMICOLON identifier DOT identifier ASSIGN string_value SEMICOLON identifier DOT identifier ASSIGN integer_value SEMICOLON {

 if(isDuplicate_struct($3) ){
struct_name($4);
...
if(strcmp($4,$6)==0 && strcmp($4,$12)==0 ){ 
struct_assign_char_value($8,$10);
struct_assign_int_value($14, $16);
noOfStructVariables++;
}else{
printf("\nERROR ON LINE %d : \nInvalid Struct! , Identifier name '%s' and Identifier name '%s' must be '%s' ",yylineno,$12,$6,$4);
exit(0); 
 }}
...
if(strcmp($5,$7)==0 && strcmp($5,$13)==0 ){ 
struct_assign_char_value($9,$11);
struct_assign_int_value($15, $17);
noOfStructVariables++;
}else{
printf("\nERROR ON LINE %d : \nInvalid Struct! , Identifier name '%s' and Identifier name '%s' must be '%s' ",yylineno,$13,$7,$5);
exit(0); 
 }}
...
switch_command: SWITCH left_parenthesis identifier right_parenthesis CASE left_parenthesis expression right_parenthesis AM commands {;}
